...
const String server = "weerlive.nl"; // Het adres van de server
const String url = "/api/json-data-10min.php"; // De url waar de informatie te vinden is
const String parameters = "?key=YOUR_KEY&locatie=" + location; // Jouw parameters
void setup() {
...
void loop() {
gegevensOphalenEnWeergeven(); // Haal de tijd op van de server en geef deze weer
delay(1000 * 60 * 10); // Wacht 10 minuten (60000 milliseconden)
}
...
client.println("GET https://" + server + url + parameters + " HTTP/1.0");
...
String temperatuur = jsonObject["liveweer"][0]["temp"].as<String>(); // Temperatuur uitlezen
display.clearDisplay(); // Maak het scherm leeg
display.setCursor(4, 20); // Begin met schrijven op positie 4,20
display.setTextSize(1); // Zet het tekstformaat op 3
display.print("Temprature in "+location+" is " + temperatuur); // Schrijf de huidige temperatuur naar het scherm
